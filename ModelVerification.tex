...
\pdfinfo{
/Title (Representation, Verification, and Visualization of Interpretations in Logic)
...
is able to interpret only (at least) the given formulae.
...
\item the TPTP language's \smalltt{\$distinct} predicate makes the domain elements distinct;
...
\item there is no specification of the domain elements because the type is defined;
...
\item for argument tuples with negative integer arguments, the interpretations of 
      functions and predicates is not specified - this is discussed more below.
\end{packed_itemize}

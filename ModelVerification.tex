...
Visualization interpretations is useful in the above areas where models are instrumental, and obviously also in teaching logic or formal methods, or as debugging tools for solver developers, for humans to understand the essence of a model.
...
The tables are naturally restricted to symbols of arity no greater than two.
The only other graphical visualization that has been found is that described in \cite{Sch13-MS},
which provides a visualization of finite first-order interpretations as produced by Paradox.
The visualization is quite different from the one in this work.
Alex - know any (other than your student)?
...
consists of non-empty domains (one for each type - just one domain in the case of untyped logic), 
and interpretations of the function and predicate symbols (of the language induced by the 
formulae) with respect to the domains \cite{Hun96}.
...
\item for each type in the formulae:
      ...
      \item the domain type, by a formula that makes the type-promotion function a surjection; 
            ...
            also \smalltt{\$int});
      \item the domain elements as terms (unless implicit from their type), as a universally 
            quantified disjunction of equalities whose right-hand sides are the domain element
            terms;
      \item specification of the distinctness of the domain elements (unless implicit from their
            type);
      \item a formula making the type-promotion function a bijection (unless the type of the 
...
\begin{enumerate}
\item This can be confirmed using standard parsing and type checking tools, e.g., \cite{VS06},
      \cite{HR15}.
\item This can be empirically confirmed using a trusted model finder (in the same way the GDV 
      ...
      theorem prover).
      Note that confirming that the \small{interpretation} formula is satisfiable is almost
      certainly much easier than finding the model itself, so the system used to check the
      satisfiability can be weaker and more trusted than the ATP system that found the model.
...
where all terms in the given and interpretation formulae are of the same type -- ``individuals'', 
thus obviating the need for type considerations.
The proof for typed first-order logic follows exactly the same pattern, but is technically
complicated due to the introduction of types, and also {\em type promotion functions} that are 
...
The extension to infinite domains is quite simple after that: if the domain is an infinite set
of uninterpreted terms they are ``enumerated'' using existential quantification. and if the
domain is defined in the TPTP language, e.g., \smalltt{\$int}, it is unnecessary to enumerate
them.
...
\begin{packed_itemize}
\item $D_I$ - A finite set of domain elements
\item $F_I$ - For each $f/n \in F_\Sigma$, a mapping $f^*: D_I^n \rightarrow D_I$
\item $R_I$ - For each $p/n \in P_\Sigma$, a mapping $p^*: D_I^n \rightarrow \{true,false\}$
\end{packed_itemize}
...
D^\vee_{\varphi_I} \land F^\wedge_{\varphi_I} \land R^\wedge_{\varphi_I}$ consists of:
\begin{equation*}
\begin{split}
...
R^\wedge_{\varphi_I} &= \bigwedge_{\substack{(\overline{d_i} \rightarrow true) \in p^* \\
                                             p^* \in P_I}} p(\overline{d_{\varphi_I,i}}) \\
...
\item For all $d \in D_{I_{\varphi_I}}$, or equivalently $d \in D_I$:
      \begin{packed_itemize}
      \item $d_{\varphi_I} \in D_{\varphi_I}$, and \\
            $X = d_{\varphi_I}$ is an element of the disjunction in $D^\vee_{\varphi_I}$
      \item With $X$ set to $d$ \\
            $I_{\varphi_I} \vdash (d = d_{\varphi_I})$ iff \\
            $d = F_{I_{\varphi_I}}(d_{\varphi_I})$ iff \\
            $d = D_{\varphi_I \rightarrow I}(d_{\varphi_I})$ iff \\
            $d = d$
      \item For $X$ set to $d$ an equality of the disjunction in $D^\vee_{\varphi_I}$ is $true$
      \end{packed_itemize}
      For all $d \in D_{I_{\varphi_I}}$ an equality of the disjunction in 
      $D^\vee_{\varphi_I}$ is $true$, i.e.,
      $I_{\varphi_I} \vdash D^\vee_{\varphi_I}$
\item For each $(f(\overline{d_{\varphi_I,i}}) = d_{\varphi_I})$ of the conjunction
      $F^\wedge_{\varphi_I}$:
      \begin{packed_itemize}
      \item $I_{\varphi_I} \vdash ( f(\overline{d_{\varphi_I,i}}) = d_{\varphi_I} )$ iff \\
            ...
            $f^*(\overline{d_i})) = d$ iff \\
            $d = d$
      \item Each equality of the conjunction $F^\wedge_{\varphi_I}$ is $true$
      \end{packed_itemize}
      $F^\wedge_{\varphi_I}$ is $true$, i.e.,
      $I_{\varphi_I} \vdash F^\wedge_{\varphi_I}$
\item For each (positive) $p(\overline{d_{\varphi_I,i}})$ of the conjunction 
      $P^\wedge_{\varphi_I}$:
      \begin{packed_itemize}
      \item $I_{\varphi_I} \vdash p(\overline{d_{\varphi_I,i}})$ iff \\
            $P_{I_{\varphi_I}}(p(\overline{d_{\varphi_I,i}}))$ iff \\
            $p^*(P_{I_{\varphi_I}}(\overline{d_{\varphi_I,i}}))$ iff \\
            $p^*((\overline{d_i})$ iff \\
            $true$
      \item Each positive atom of the conjunction $P^\wedge_{\varphi_I}$ is $true$
      \end{packed_itemize}
      Analogously, each (negative) $\neg p(\overline{d_{\varphi_I,i}})$ of the conjunction 
      $P^\wedge_{\varphi_I}$ is $false$, and thus
      $P^\wedge_{\varphi_I}$ is $true$, i.e., $I_{\varphi_I} \vdash P^\wedge_{\varphi_I}$
\end{itemize}
\end{proof}
...
IDV \cite{TPS07}.
However, interpretation visualization has (to the knowledge of the authors) had minimal attention,
as noted in Section~\ref{Introduction}. 
A way of visualizing interpretations for TFO formulae has been designed in this work, and 

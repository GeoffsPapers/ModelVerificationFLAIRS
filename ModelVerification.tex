...
The SMT-LIB format~\cite{BFT17} provides facilities to inspect or output models.  SAT solvers generally output models as specified by the SAT competitions, in a simple format similar to the DIMACS input format.
...
The TX0 types are
\marginpar{PF: TX0: is this intentional or a typo for TF0?}
...
{\em type-promotion} function is declared, which \marginpar{PF: I guess the previous incomplete sentence must be deleted: there is info about promotion below}
...
are the parents\marginpar{PF:hypotheses?} of the inference and whose conjecture is the conclusion of the inference,

...
This process is depends on finding an {\em interpretation} of the formulae, i.e., structures that
map expressions written in the language of the formulae to domain elements and truth values.
In most cases the interpretation is a {\em model} of certain formulae, i.e., it maps the those
formulae to {\em true}.
...
This papers examines the tasks of verifying models and visualizing interpretations, and describes 
...
Other formats for interpretations include the SMT format PASCAL TO FILL IN, the DIMACS format,
...
Succ(x) =X+ 1
Works when domain is integer. Currently z3 does not implement infinite models for uninterpreted sorts. I would probablybsupport infinite sorts by creating injection into annalgebraic datatype and then support models that can be expressed over ADT.
...
This is the approach take in, e.g., SPASS \cite{WF+09} and in GDV \cite{Sut06}.
The same approach can be taken to verify a model of given formulae - prove each of the given 
formulae using the model formula as an axiom.
The soundness of this approach relies the fact that if a given formula can be proven from
the model formula, then the given formula is interpreted as {\em true} by the model represented

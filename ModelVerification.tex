...
\noindent
In the TPTP language, an interpretation is represented by an {\em interpretation formula}
$\varphi_I$.
Let:
\begin{packed_itemize}
\item $D_{\varphi_I}$ be a set of fresh terms $d^I_i$, one for each $d_i \in D_I$
\item $D_{I \rightarrow \varphi_I}$ be the corresponding mapping from $D_I$ to $D_{\varphi_I}$
\item $D_{\varphi_I \rightarrow I}$ be the inverse mapping from $D_{\varphi_I}$ to $D_I$
\end{packed_itemize}

\noindent
Let $\Sigma_{\varphi_I}$ be an untyped first-order language:
\begin{packed_itemize}
...
\noindent
The interpretation formula $\varphi_I \in \mathcal{F}(\Sigma_{\varphi_I}) = 
D^\vee_{\varphi_I} \land F^\wedge_{\varphi_I} \land R^\wedge_{\varphi_I}$ consists of:
\begin{equation*}
\begin{split}
D^\vee_{\varphi_I}   &= \forall X \bigvee_{d^I_i \in D_{\varphi_I}} \left(X = d^I_i \right) \\
F^\wedge_{\varphi_I} &= \bigwedge_{\substack{(d_i^n \rightarrow d) \in f^* \\ 
                                             f^* \in F_I}} f({d^I_i}^n) = d^I) \\
R^\wedge_{\varphi_I} &= \bigwedge_{\substack{(d_i^n \rightarrow true) \in p^* \\
                                             p^* \in P_I}} p({d^I_i}^n) \\
                     &\bigwedge \bigwedge_{\substack{(d_i^n \rightarrow false) \in p^* \\
                                             p^* \in P_I}} \neg p({d^I_i}^n)
\end{split}
\end{equation*}
...
\begin{itemize}
\item For all $d_i \in D_{I_{\varphi_I}}$, or equivalently $d_i \in D_I$,
      \begin{packed_itemize}
      \item $d^I_i \in D_{\varphi_I}$, and \\
            $X = d^I_i$ is an element of the disjunction in $D^\vee_{\varphi_I}$
      \item With $X$ set to $d_i$ \\
            $I_{\varphi_I} \vdash d_i = d^I_i$ iff \\
            $F_{I_{\varphi_I}}(d_i) = F_{I_{\varphi_I}}(d^I_i)$ iff \\
            $F_{I_{\varphi_I}}(d_i) = D_{\varphi_I \rightarrow I}(d^I_i)$ iff \\
            $d_i = d_i$
      \item Thus an element of of the disjunction in $D^\vee_{\varphi_I}$ is $true$
      \end{packed_itemize}
      So for all $d_i \in D_{I_{\varphi_I}}$ an element of of the disjunction in 

...
The axioms and conjecture to be proved (and hence become a theorems) are written in an 
...
This process depends on finding an {\em interpretation}, i.e., a structure that maps expressions 
to domain elements and truth values.
If an interpretation maps formulae to {\em true} then it is a {\em model} of the formulae.
...
output by Nitpick and Z3 \cite{dMB08}.
% +++
% Nikolaj says ...
...
axioms.
Personal communication with members of the Vampire team revealed that Vampire can internally 
verify finite models using a direct approach, which uses the model formulae to evaluate (rather 
than prove) the input formulae.
...
This is weaker than the approach taken here and that taken by Vampire.
...
information about the models it finds, including the interpretations for constants as integer 
domain elements, and tables for the function and predicate symbols' interpretations. 
...
The language is used for writing both problems and solutions, which enables convenient 
...
TF0 \cite{SS+12}, the monomorphic form of TFF, described in Section~\ref{TF0}, 
...
(iii)~user-defined types of the kind {\smalltt{\$tType}}.
...
(just one domain for untyped logic), and interpretations of the 
...
In addition to Tarskian-style interpretations,
% that provide an explicit interpretation of the symbols, 
a Herbrand model 
% of clausal formulae 
can also be embodied in a saturation \cite{BG+01}.
...
Saturations are difficult to interpret in a way that can be used constructively by 
users, and are thus a less useful form of interpretation.
This work considers 
% the representation, verification, and visualization of 
only Tarskian-style interpretations with explicit symbol interpretation.
...
\item the type declarations for the formulae being interpretted;
...
\item for each type in the formulae:
      ...
      \item the domain elements as terms (unless implicit from their type): if the domain is
            ...
            right-hand sides are the terms; if the domain is infinite an existentially quantified 
            formula that captures an infinite disjunction of equalities is used, e.g., for terms 
...
%            \smalltt{! [I: peano] : ( I = zero | ? [P: peano] : I = s(P) )};
      ...
      \item a formula making the type-promotion function an injection,
            ...
            which with surjectivity makes it a bijection.
      \end{packed_itemize}
\item interpretation of the function symbols, using equalities whose left-hand sides are 
...
tuples with negative integers - this is an example of a partial interpretation.
...
\item In this work a ``semantic'' approach is taken, in which th given formulae are proved from
      ...
      supplied as an axiom, and the given formulae are the conjecture to be proved.
...
complicated due to the introduction of types and the type promotion functions.
...
$I_{\varphi_I} \vdash P^\wedge_{\varphi_I}$. Each part can be checked separately:
\begin{itemize}
...
This visualization is available in the IIV tool in SystemOnTSTP, using 
{\scriptsize \url{https://raw.githubusercontent.com/GeoffsPapers/ModelVerification/main/TFF_Finite.s.IIV}}
...
This visualization is available in the IIV tool using 
...
At this stage the implementation is prototype in the sense that different components of the 
interpretation formula have to be manually extracted into separate annotated formulae, to 
mimic a derivation that IDV can render.
...
A tool to translate interpretation formulae to the format required for input to the IIV tool is 
being implemented.
